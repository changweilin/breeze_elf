...
| `BREEZE_ASR_FILE_BEAM` | `5` | Beam width for the batched whole-file endpoint. `5` (default) reflects the §2 decode sweep, where beam 5 beat greedy on sung audio (MIR-1K CER −16%, Jamendo WER lower) at ~2–3× the decode cost — worth it offline. Set `1` for greedy/fastest. Live-mic streaming stays at beam 1 (fixed) so latency is untouched. |
