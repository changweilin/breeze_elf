...
function demoCharacters(text, startSeconds, endSeconds, jianpu) {
  ...
  // Cycle the note values so 簡譜檢視 demonstrates every 時值 symbol (底線/附點/延長線).
  const noteCycle = [
    [0.25, "十六分音符"],
    [0.5, "八分音符"],
    [1.0, "四分音符"],
  ...
  return chars.map((char, index) => {
    ...
    const [beats, noteValue] = noteCycle[index % noteCycle.length];
    return {
      char,
...
// 時值 → 簡譜裝飾:底線數(八分/十六分)、附點、延長線(每延一拍補一條 "–")。
// beats 就是 audio.py `_NOTE_VALUES` 的第一欄,payload 直接帶過來。
const JIANPU_DURATIONS = [
  { beats: 4.0, underlines: 0, dotted: false, dashes: 3 },
  { beats: 3.0, underlines: 0, dotted: false, dashes: 2 },
  { beats: 2.0, underlines: 0, dotted: false, dashes: 1 },
  { beats: 1.5, underlines: 0, dotted: true, dashes: 0 },
  { beats: 1.0, underlines: 0, dotted: false, dashes: 0 },
  { beats: 0.75, underlines: 1, dotted: true, dashes: 0 },
  { beats: 0.5, underlines: 1, dotted: false, dashes: 0 },
  { beats: 0.375, underlines: 2, dotted: true, dashes: 0 },
  { beats: 0.25, underlines: 2, dotted: false, dashes: 0 },
];
const JIANPU_NO_DURATION = { underlines: 0, dotted: false, dashes: 0 };

function jianpuDuration(beats) {
  if (!Number.isFinite(beats) || beats <= 0) {
    return JIANPU_NO_DURATION;
  }
  // Snap to the nearest note value in log space (same metric as quantize_beats),
  // so a rounded payload value still resolves to exactly one symbol set.
  let best = JIANPU_NO_DURATION;
  let bestDelta = Infinity;
  for (const entry of JIANPU_DURATIONS) {
    const delta = Math.abs(Math.log(beats / entry.beats));
    if (delta < bestDelta) {
      bestDelta = delta;
      best = entry;
    }
...
function renderJianpuLine(characters) {
  ...
  characters.forEach((character) => {
    ...
    top.className = "jp-top";
    const jp = document.createElement("span");
    jp.className = dur.underlines ? `jp underline-${dur.underlines}` : "jp";
    jp.textContent = character.jianpu || "·";
    top.append(jp);
    if (dur.dotted) {
      // 附點:延長本音一半,寫在數字右側(不畫底線)。
    ...
    cell.append(top, ch);
    if (Number.isFinite(character.hz)) {
      const note = character.noteValue ? ` · ${character.noteValue}` : "";
      cell.title = `${character.char} · ${Math.round(character.hz)} Hz${note}`;
    }
    line.append(cell);

    // 延長線:每延一拍補一條 "–" 佔一欄,沿用簡譜的橫線記法。結構與音符欄一致
    // (.jp-top 包住數字 + 佔位空白的 .ch),延長線才會落在數字列而非掉到字元列。
    for (let i = 0; i < dur.dashes; i += 1) {
      const dash = document.createElement("span");
      dash.className = "jianpu-char dash";
      const dashTop = document.createElement("span");
      dashTop.className = "jp-top";
      const dashJp = document.createElement("span");
      dashJp.className = "jp";
      dashJp.textContent = "–";
      dashTop.append(dashJp);
      const dashCh = document.createElement("span");
      dashCh.className = "ch";
      dashCh.textContent = String.fromCharCode(0xa0);
      dash.append(dashTop, dashCh);
      line.append(dash);
    }
  });
  return line;
